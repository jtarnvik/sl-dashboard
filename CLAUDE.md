...
1. BE — DTO naming convention. Classes used as DTOs (including response records) should be named with a `Dto` suffix, or not — what is idiomatic in Spring Boot? Add a rule to the backend CLAUDE.md once decided.

---

2. BE — Design/Discuss: Should there be a dedicated `dto` package?

---

3. BE — Use MapStruct for entity-to-DTO mapping. Add MapStruct and the MapStruct Lombok extension as dependencies. Add a rule to the backend CLAUDE.md.

---

4. FE — The `backend.ts` communication functions are becoming repetitive. Create a generic fetch helper that accepts a URL and optional payload and returns a typed promise with built-in error handling. Investigate what is idiomatic in a React/TypeScript project.

---

5. FE — File naming: rename admin view files to kebab-case. `ExistingUsers.tsx` → `existing-users.tsx`, `PendingUsers.tsx` → `pending-users.tsx`.

---

6. FE — Shared user row component and action enum. Create a reusable component for a user row, and a component for the action buttons that takes an enum argument controlling which actions are active.

---

7. FE — Add a divider line above "Logga ut" in the hamburger menu.

---

8. FE — Make the admin views responsive and mobile-friendly. The current table layout does not work on small screens. Investigate a grid-based approach with different layouts per viewport (a list on mobile, table on desktop). Add a rule to CLAUDE.md that the app should be responsive and mobile-first.

---

9. FE/BE — Enhance the admin hamburger menu with pending user count.
...
10. Discuss/Decide — Should rejected access request users be notified?
...
11. BE — Scheduled cleanup of stale pending login attempts.
...
12. Investigate/Discuss: When/If shall the Google login be changed from test mode.

---

13. BE — Should the API types be sorted into their own folder by type?

---

14. Design/Discuss — API authorization strategy.
...
15. FE/BE — Logged-in users should have their settings stored in the database.
